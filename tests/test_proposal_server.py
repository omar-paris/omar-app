...
def test_public_audit_payload_renderers_escape_malicious_payloads():
    malicious = '<img src=x onerror="window.__ATHENA_XSS=1">'
    onboarding_html = (ROOT / "pages-app" / "onboarding.html").read_text(encoding="utf-8")
    devis_html = (ROOT / "pages-app" / "devis.html").read_text(encoding="utf-8")

    assert malicious not in onboarding_html
    assert "function textEl" in onboarding_html
    assert "textContent=String(text" in onboarding_html
    assert "box.replaceChildren(...nodes)" in onboarding_html
    assert "buildRecap(\"recap_final\")" in onboarding_html
    assert "document.getElementById(\"recap_final\").innerHTML" not in onboarding_html
    assert "${s.summary||" not in onboarding_html

    assert malicious not in devis_html
